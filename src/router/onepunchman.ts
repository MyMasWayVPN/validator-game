import { endpoint, headers, Result } from '../utils'

export default async function onepunchman(id: string, zone: number): Promise<Result> {
  let dataid: string[] = id.split(_)
  const JWT1 = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..Ah9LA23iwtpoEEMORZ5giMSjgQ6MUDkAkwZifkpILjU"
  const JWT2 = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..-Hs_g0kLoX41k5xgq7j9jScMzg3I-TlMAu5qj0U0Af4"
  const body = `voucherPricePoint.id=308475&voucherPricePoint.price=44000.0000&voucherPricePoint.variablePrice=0&user.userId=${dataid[0]}_${dataid[1]}&user.zoneId=${zone}&voucherTypeName=ONE_PUNCH_MAN&lvtId=11840&shopLang=id_ID`
  const response = await fetch(endpoint, {
    method: 'POST',
    headers,
    body
  })
  const data = await response.json()
  if (data.succes == true) {
    return {
      success: true,
      game: 'ONE_PUNCH_MAN',
      id,
      name: data.confirmationFields.username
    }
  }
  else {
    return {
      success: false,
      message: "Not found"
    }
  }
} 
