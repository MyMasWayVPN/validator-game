...
export default async function onepunchman(id: string, zone: number): Promise<Result> {
  const datam: string = `${id}`
  const dataid: string[] = datam.split(_)
  const hasilid: string = `${dataid[0]}_${dataid[1]}`
  ...
  const body = `voucherPricePoint.id=308475&voucherPricePoint.price=44000.0000&voucherPricePoint.variablePrice=0&user.userId=${hasilid}&user.zoneId=${zone}&voucherTypeName=ONE_PUNCH_MAN&lvtId=11840&shopLang=id_ID`
